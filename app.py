...
st.title("📊 CPTU → Excel")
st.caption("Wrzuć kartę CPTU (PDF z wykresami wektorowymi, np. z CPT-Star) — dostaniesz gotowy Excel z danymi i wykresami.")
...
if uploaded is not None:
    with st.spinner("Czytam wykresy z PDF..."):
        try:
            ...
            st.info(
                "Najczęstsza przyczyna: PDF nie zawiera wektorowych wykresów "
                "(np. jest skanem/zdjęciem) albo ma inny układ paneli niż standardowy "
                "qc / fs / u2 / Rf(qc)."
            )
            st.stop()

    st.success(f"Gotowe — odczytano {len(df)} wierszy, głębokość {df['Głębokość [m]'].min():.2f} do {df['Głębokość [m]'].max():.2f} m.")
